...
function preload() {
    ...
    winImg = loadImage("you_win-removebg-preview.png");
}
function setup() {
...
function draw() {
    ...
    if (gameState === PLAY) {
        ...
        if (!luaCriada && frameCount > 10 * 10) {
            lua = createSprite(width + 100, height / 2);
            lua.addImage(luna);
            lua.velocityX = -8;
            luas.add(lua);
            luaCriada = true;
        ...
        if (luas.isTouching(jetbubs)) {
            win.visible = true;
            luas.setVelocityXEach(0);
            meteor5.destroyEach();
            meteor4.destroyEach();
            meteor3.destroyEach();
            meteor2.destroyEach();
            meteor1.destroyEach();
            win.depth = lua.depth + 1;
        }
